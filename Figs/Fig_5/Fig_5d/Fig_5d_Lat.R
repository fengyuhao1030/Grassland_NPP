...
dataMatrix <- read.csv('fBNPP_LatSum.csv')
...
Fig_3d_Lat <- ggplot()+
  ...
  scale_y_continuous(limits = c(0.3,0.85),breaks = seq(0.4,0.8,0.2),labels = c('0.4','0.6','0.8'),expand = c(0,0))+
  ylab('fBNPP')+
  coord_flip()+
  theme_custom()
pdfName <- paste0(dirname(rstudioapi::getActiveDocumentContext()$path),'\\Fig_3d_Lat.pdf')
